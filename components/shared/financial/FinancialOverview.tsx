...
// 1. ปรับ Props ให้รับ apiUrl ได้
export type FinancialOverviewProps = {
  initialData?: Overview;
  apiUrl?: string; // เช่น "/api/executive/financial-overview" หรือ "/api/superadmin/financial-overview"
};

export default function FinancialOverview({ initialData, apiUrl }: FinancialOverviewProps = {}) {
    ...
    fetch(`${apiUrl}?year=${selectedYear}`, { signal: controller.signal })
      ...
      .catch((err: unknown) => {
        if (!(err instanceof DOMException && err.name === "AbortError")) {
          console.error("Failed to load financial overview from API", err);
          setError("ไม่สามารถโหลดข้อมูลสถิติทางการเงินได้ กรุณาตรวจสอบการเชื่อมต่อ API");
        }
      });
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50 p-8 text-center text-red-500 font-[family-name:var(--font-kanit)]">
